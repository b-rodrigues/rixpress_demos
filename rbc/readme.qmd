...
In an interactive session, one can use `rxp_read()`, `rxp_load()` to read or load
artifacts into the R session respectively and `rxp_trace()` to check the lineage of 
an artifact. The pipeline could also be built from a Python session, using `ryxpress`,
the Python port of `{rixpress}`.
